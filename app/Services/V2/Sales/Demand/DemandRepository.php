...
//            }
    ...
    public function getDemandJointOwnersById(string $id, User $user)
    {
        $data = $this->dynamicsCrmClient->getDemandById($id, $user);
    ...
    private function makeDemand(array $data, User $user): Demand
    {
        ...
        if ($data['customerType']['code'] == 1) {
            $customer = $this->customerRepository->getByAccountId($data['customerId']);
        ...
        if (isset($data['depositorFizId'])) {
            foreach ($data['jointOwners'] as $key => $jointOwner) {
                if ($jointOwner['id'] == $data['depositorFizId']) {
        ...
        if (isset($data['articleOrders'])) {
            ...
            if ((($data['demandSubType']['code']??null) == DemandBookingType::tradeIn()->value || isset($article['serviceCode'])) && ($article['serviceCode']??null) == '020020') {
                $tradeIn = new TradeIn(
        ...
        if (isset($data['paymentPlan'])) {
            if ((($data['bindedCharacteristicSales']??null) == null && count($data['paymentPlan']) > 1) ||
                (($data['bindedCharacteristicSales']??null) == null && ($data['articlePrice'] != $data['sumOpportunityMinusDiscount']))) {
        ...
        if ($isAvailableChangePaymentMode == true) {
            foreach ($data['bindedCharacteristicSales']??[] as $bindedCharacteristicSale) {
                if ($bindedCharacteristicSale['processingType']['code'] == 16 && ($data['articlePrice'] != $data['sumOpportunityMinusDiscount'])) {
                    $isAvailableChangePaymentMode = false;
                } else {
                    $isAvailableChangePaymentMode = true;
                    break;
        ...
        if ($data['demandSubType']['code'] == 1) {
            ...
            $paymentBooking['paid_booking_expiry_time'] = $project?$project->paid_booking_expiry_time:7;
            ...
            $paymentBooking['booking_payment_time'] = null;
                ...
                ($data['contractReservPaymentStatus']['code'] ?? null) == 1) && $data['stepName'] != 'Бронирование') {
