...
class StagesRepository
{
    ...
    public function makeContractStages(array $demand, array $characteristics = [], ?bool $isContractApprove = null): array
    {
        ...
        } elseif ((($demand['stepName'] ?? '') == 'Подготовка пакета документов') && $isContractApprove === null) {
            $stages[0]['status'] = 'done';
            $stages[1]['status'] = 'wait';
            $stages[1]['message'] = 'Ожидайте подготовки договора';
            $stages[2]['status'] = 'closed';
    ...
    public function makeContractStagesV2(array $demand, ?array $contracts, array $characteristics = [], ?bool $isContractApprove = null, ?Property $property = null): array
    {
        // phpcs:disable
        $saleStages = [];
        ...
        if ($contracts != []) {
            //Для 090010
            $contractsCollection = collect($contracts)->filter(function ($contract) use ($demand) {
                /** @var Contract $contract */
                return $contract->getService()?->value == '090010' && $contract->getOpportunityMainId() == $demand['id'];
            });

            $activeContract = null;
            foreach ($contractsCollection as $contract) {
                try {
                    $activeContract = $this->dynamicsCrmClient->getContractById($contract->getId());
            ...
            if (($activeContract['registrationNumber'] ?? null) != null) {
                $stages[6]['status'] = 'done';
            } elseif ($activeContract != null) {
                $stages[6]['status'] = 'active';
            }

            //Для 090020, 030080, 030090
            $contractsCollection = collect($contracts)->filter(function ($contract) use ($demand) {
                /** @var Contract $contract */
                return in_array($contract->getService()->value, ['090020', '030080', '030090']) && $contract->getOpportunityMainId() == $demand['id'];
            });
            $activeContract = null;

            foreach ($contractsCollection as $contract) {
                try {
                    $activeContract = $this->dynamicsCrmClient->getContractById($contract['id']);
                } catch (\Throwable $exception) {
                    $activeContract = [];
                    $stages[4]['status'] = 'active';
                    $stages[4]['message'] = 'Подписать';
                }
            }

            $countOfContract = collect($activeContract)->filter(function ($item) {
                return isset($item->registrationNumber);
            })->count();

            if ($activeContract != null && $countOfContract == count($activeContract)) {
                $stages[4]['status'] = 'done';
            } elseif ($countOfContract != 0) {
                $stages[4]['status'] = 'active';
                $stages[4]['message'] = 'Подписать';
            }
        }
        ...
        if ($property != null) {
            switch ($property->getArticleStatusReception()) {
                case 28:
                case 1:
                case 3:
                case 8:
            ...
            try {
                $inspection = $this->dynamicsCrmClient->inspection($property->getId());
            } catch (\Throwable $exception) {
                $inspection = null;
            }

            if ($inspection['data'] ?? false) {
                $data = collect($inspection['data'])->filter(function ($inspect) {
                    return in_array($inspect['status_id'], [1,2,3,9]) && $inspect['type_id'] == 1;
                })->first();

                $dataZam = collect($inspection['data'])->filter(function ($inspect) {
                    return in_array($inspect['status_id'], [5]) && $inspect['type_id'] == 1;
                })->first();

                if ($data && $property->getArticleStatusReception() == 4) {
                    $date = $data['take_date_start'] ? Carbon::parse($data['take_date_start'])->addHours(3)->format('d.m.Y H:i') : '';
                    $stages[5]['message'] = 'Запись на ' . $date;
                } elseif ($dataZam && $property->getArticleStatusReception() == 5) {
                    $stages[5]['message'] = 'Устранение замечаний';
                } elseif ($property->getArticleStatusReception() == 2) {
                    $stages[5]['message'] = 'Открыта запись на приемку';
                }
            }
        }

        if ($stages[4]['status'] == 'active' || $stages[4]['status'] == 'wait') {
            $stages[4]['icon'] = Storage::url('public/images/icons/stages/dop_soglasheniya_active.png');
        } else {
            $stages[4]['icon'] = Storage::url('public/images/icons/stages/dop_soglasheniya_disabled.png');
        }

        if ($stages[5]['status'] == 'active' || $stages[5]['status'] == 'wait') {
            $stages[5]['icon'] = Storage::url('public/images/icons/stages/doraschet_active.png');
        } else {
            $stages[5]['icon'] = Storage::url('public/images/icons/stages/doraschet_disabled.png');
        }

        if ($stages[6]['status'] == 'active' || $stages[6]['status'] == 'wait') {
            $stages[6]['icon'] = Storage::url('public/images/icons/stages/priemka_active.png');
        } else {
            $stages[6]['icon'] = Storage::url('public/images/icons/stages/priemka_disabled.png');
        }
    ...
    public function makeContractStagesV3(array $demand, ?array $contracts, array $characteristics = [], ?bool $isContractApprove = null, ?Property $property = null): array
    {
        // phpcs:disable
        $saleStages = [];
        ...
        if ($this->getHideUkDocuments($property)) {
            $stages[6]['status'] = 'closed';
        }
    ...
    public function MakeAdditionalContractStages(Contract $contracts): array
    {
        ...
        if ($contracts->getStepName() == 'Подписание договора' || $contracts->getStepName() == 'Подготовка пакета документов') {
            $stages[0]['status'] = 'active';
            $stages[1]['status'] = 'active';
        } elseif ($contracts->getStepName() == 'Регистрация' || $contracts->getStepName() == 'Оплата') {
            $stages[0]['status'] = 'done';
            $stages[1]['status'] = 'active';
        } elseif (($contracts->getRegistrationDate() != null && $contracts->getRegistrationNumber() != null) || $contracts->getRegistrationStage() == 'Договор зарегистрирован') {
            $stages[0]['status'] = 'done';
            $stages[1]['status'] = 'done';
        }

        if (($stages[0]['status'] ?? '') == 'closed') {
            $stages[0]['icon'] = Storage::url('public/images/icons/substages_additional/ep_disabled.png');
        } else {
            $stages[0]['icon'] = Storage::url('public/images/icons/substages_additional/ep_active.png');
        }

        if (($stages[1]['status'] ?? '') == 'closed') {
            $stages[1]['icon'] = Storage::url('public/images/icons/substages_additional/podpisanie_disabled.png');
        } else {
            $stages[1]['icon'] = Storage::url('public/images/icons/substages_additional/podpisanie_enabled.png');
        }
