...
use App\Services\Payment\PSBPaymentService;
 ...
 */
class PSBPaymentController extends Controller
{
    /**
    ...
    public function callback(
        ...
        $transactionLog = TransactionLog::where('psb_order_id', '=', $request->input('ORDER'));
        $paymentMethod = 'PSB';

        $path = storage_path('logs/psb_payments.log');
        ...
        try {
            if ($transactionLog->value('claim_id')) {
                if ($request->input('RESULT') == 0) {
                    ...
                    $path = storage_path('logs/failed_psb_payments.log');
                    $text = date('Y-m-d H:i:s').' | request: '. $_SERVER['REQUEST_URI']. " | " .
                        $request->input('ORDER');
    ...
    private function getLogger()
    {
        ...
        $filePath = 'psb_payments_' . $dateString . '.log';
